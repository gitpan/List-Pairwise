...
The old behavior can be restored by either making these misc warnings FATAL:
...
The author wishes to thank:
    - Johan Lodin for the pair() idea and implementation, as well as the idea of
      warning instead of dying when given an odd number of arguments, to mimic
      perl behavior
    - Andreas J. Koenig for its advices on documentation and its insight on how to
